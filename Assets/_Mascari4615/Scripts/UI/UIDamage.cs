...
public class UIDamage : MonoBehaviour
{
	...
	private (Animator animator, TextMeshProUGUI tmp) Pop()
	{
		if (_damageUIs.Count == 0)
		{
			for (int i = 0; i < 10; i++)
			{
				GameObject newObject = Instantiate(damageTextPrefab, damageTextsRoot);
				Animator damageUIAnimator = newObject.GetComponent<Animator>();
				damageUIAnimator.keepAnimatorStateOnDisable = true;
				damageUIAnimator.gameObject.SetActive(false);
				_damageUIs.Push((damageUIAnimator, damageUIAnimator.transform.GetChild(0).GetComponent<TextMeshProUGUI>()));
			}
